...
(
  ...
  -- optional clustering cost per pricing tier
  clustering_cost   Nullable(Float64),
...
def sql_nullable_num(n):
  if n == null then "NULL" else (n | tostring) end;
...
| (($r.costs // []) | map(
    ...
    | sql_nullable_num($c.clustering_cost) as $clustering_cost_
    ...
    | "("
      ...
      + $compute_costs_ + "," + $clustering_cost_ + "," + $storage_cost_num_ + "," + $storage_costs_arr_
    + ")"
  )) as $rows
...
while IFS= read -r -d '' f; do
  ...
  if ! $CH_CLIENT --query "
      INSERT INTO ${DB}.${TABLE} (
        ...
        compute_costs, clustering_cost, storage_cost, storage_costs
      )
      VALUES ${values_sql};
  " </dev/null; then
    echo "   ⚠️  Failed: $f" >&2
