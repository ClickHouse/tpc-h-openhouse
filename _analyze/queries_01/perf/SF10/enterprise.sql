...
(
    ...
    SELECT '19' AS id, 'Snowflake Ent (4X-L)' AS bar_label, 'Snowflake%' AS system_pat,
        ...
        UNION ALL
    -- 08: Snowflake Enterprise Gen2 4X-Large (172.8 credits/h)
    SELECT '20' AS id, 'Snowflake Ent (Gen2 4X-L)' AS bar_label, 'Snowflake%' AS system_pat,
           'enterprise' AS tier, NULL AS compute_model,
           'aws' AS provider, 'us-east-1' AS region,
...
rows AS (
  ...
  (
      ...
      FROM bench2cost_tcph_sf10.costs
  ) c
