...
# --------- config ----------
QUERY_INPUT="${1:-queries.sql}"  # file (split on `;`) or directory of *.sql files
DATASET="${DATASET:-}"           # optional: default dataset for unqualified table refs
...
if [[ -d "$QUERY_INPUT" ]]; then
  # Directory mode: each *.sql file is one query (multi-statement scripts ok).
  shopt -s nullglob
  FILES=("$QUERY_INPUT"/*.sql)
  shopt -u nullglob
  (( ${#FILES[@]} > 0 )) || { echo "ERROR: no .sql files in $QUERY_INPUT" >&2; exit 1; }
  for f in "${FILES[@]}"; do
    QUERIES+=("$(<"$f")")
  done
elif [[ -f "$QUERY_INPUT" ]]; then
  # File mode: split on `;`, NUL-delimit so multi-line queries survive.
  mapfile -d '' -t QUERIES < <(awk '
    BEGIN{RS=";"; ORS=""}
    { q=$0; gsub(/^[ \t\r\n]+|[ \t\r\n]+$/,"",q); if (length(q)>0) printf "%s%c", q, 0 }
  ' "$QUERY_INPUT")
else
  echo "ERROR: $QUERY_INPUT is neither a file nor a directory" >&2; exit 1
fi

(( ${#QUERIES[@]} > 0 )) || { echo "ERROR: No queries found in $QUERY_INPUT" >&2; exit 1; }
...
run_one() {
  ...
  local DS_FLAG=()
  [[ -n "$DATASET" ]] && DS_FLAG=(--dataset_id="$DATASET")
...
mkdir -p "$RESULTS_DIR"
OUT_FILE="$RESULTS_DIR/bq${DATASET:+_${DATASET}}_${TS}.json"
