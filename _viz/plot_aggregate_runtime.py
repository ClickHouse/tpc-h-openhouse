...
def main():
    ...
    parser.add_argument("--no-y-axes", action="store_true")
    parser.add_argument("--no-grid", action="store_true")
    parser.add_argument("--no-footer", action="store_true")
    parser.add_argument("--no-border", action="store_true")
    parser.add_argument("--sort", action="store_true", help="Sort bars by runtime ascending.")
    parser.add_argument("--show-values", action="store_true", help="Show runtime labels above bars.")
    parser.add_argument("--cost-line", action="store_true", help="Overlay cost_hot as a line on a secondary y-axis.")
    parser.add_argument("--show-cost-values", action="store_true", help="Show cost labels next to the cost-line markers.")
    ...
    if args.cost_line:
        df["cost_hot"] = df["cost_hot"].astype(float)
    ...
    if args.cost_line:
        ...
        ax2.plot(
            ...
            marker="o",
            markersize=7,
            markerfacecolor=COST_LINE_COLOR,
            markeredgecolor=bg,
            markeredgewidth=1.2,
            zorder=5,
        )
        ...
        if args.no_y_axes:
            ...
            ax2.set_ylabel("Cost, USD — hot run", color=muted, fontsize=13)
            ax2.tick_params(axis="y", colors=muted, labelsize=11)
        ...
        if args.show_cost_values:
            ymax_cost = df["cost_hot"].max()
            for xpos, cost in zip(x, df["cost_hot"]):
                ax2.text(
                    xpos,
                    cost + ymax_cost * 0.035,
                    fmt_cost(cost),
                    ha="center",
                    va="bottom",
                    color=COST_LINE_COLOR,
    ...
    if not args.no_footer:
        note = "Lower is better. Runtime is the sum of the fastest run for each of the 22 TPC-H queries."
        if args.cost_line:
            note += " The line shows hot-query cost."
    ...
    bottom = 0.08 if args.no_footer else 0.13
    plt.tight_layout(rect=[0.02, bottom, 0.98, 0.94])
    fig.savefig(
        args.out,
        dpi=180,
